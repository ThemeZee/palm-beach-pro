...
class Palm_Beach_Pro_Admin_Notices {
	...
	static function activate_license() {
		...
		if ( '' === $options->get( 'license_key' ) && in_array( $pagenow, array( 'index.php', 'update-core.php', 'plugins.php', 'themes.php' ) ) && ! isset( $_GET['page'] ) && current_theme_supports( 'palm-beach-pro' ) && ! get_transient( 'palm_beach_pro_activate_license_dismissed' ) && current_user_can( 'edit_theme_options' ) ) : ?>
	...
	static function missing_theme() {
		...
		if ( ! current_theme_supports( 'palm-beach-pro' ) && in_array( $pagenow, array( 'index.php', 'update-core.php', 'plugins.php', 'themes.php' ) ) && ! isset( $_GET['page'] ) && ! get_transient( 'palm_beach_pro_missing_theme_dismissed' ) && current_user_can( 'edit_theme_options' ) ) : ?>

			<div class="notice notice-warning">
				<p>
					<?php printf( __( 'The %1$s add-on needs the %2$s theme activated in order to work. You should deactivate %1$s if you have switched to another theme permanently.', 'palm-beach-pro' ),
						PALM_BEACH_PRO_NAME,
						'Palm Beach'
					); ?>
					<a style="float: right" href="<?php echo wp_nonce_url( add_query_arg( array( 'palm_beach_pro_action' => 'dismiss_notices', 'palm_beach_pro_notice' => 'missing_theme' ) ), 'palm_beach_pro_dismiss_notice', 'palm_beach_pro_dismiss_notice_nonce' ); ?>"><?php _e( 'Dismiss Notice', 'palm-beach-pro' ); ?></a>
				</p>
