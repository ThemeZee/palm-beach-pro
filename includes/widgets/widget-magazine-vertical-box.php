 ...
 */
class Palm_Beach_Pro_Magazine_Vertical_Box_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'palm-beach-magazine-vertical-box', // ID.
			esc_html__( 'Magazine: Vertical Box', 'palm-beach-pro' ), // Name.
			array(
				'classname' => 'palm_beach_magazine_vertical_box',
				'description' => esc_html__( 'Displays your posts from a selected category in a vertical box layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'palm-beach-pro' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_palm_beach_magazine_vertical_box', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_palm_beach_magazine_vertical_box', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	...
	function widget_title( $args, $settings ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $settings['category'] > 0 ) :
				...
				$link_title = sprintf( __( 'View all posts from category %s', 'palm-beach-pro' ), get_cat_name( $settings['category'] ) );
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_palm_beach_magazine_vertical_box', 'widget' );
