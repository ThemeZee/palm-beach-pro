 ...
 */
class Palm_Beach_Pro_Magazine_Posts_Boxed_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'palm-beach-magazine-posts-boxed', // ID.
			sprintf( esc_html__( 'Magazine Posts: Boxed (%s)', 'palm-beach-pro' ), 'Palm Beach Pro' ), // Name.
			array(
				'classname' => 'palm_beach_magazine_posts_boxed',
				'description' => esc_html__( 'Displays your posts from a selected category in a boxed layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'palm-beach-pro' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_palm_beach_magazine_posts_boxed', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_palm_beach_magazine_posts_boxed', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
					...
					<div class="magazine-grid-post clearfix">
						...
						<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
							...
							<div class="entry-content entry-excerpt clearfix">
								...
								<a href="<?php echo esc_url( get_permalink() ) ?>" class="more-link"><?php esc_html_e( 'Read more', 'palm-beach-pro' ); ?></a>
	...
	function widget_title( $args, $settings ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $settings['category'] > 0 ) :
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'palm-beach-pro' ), get_cat_name( $settings['category'] ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'layout' ); ?>"><?php esc_html_e( 'Post Layout:', 'palm-beach-pro' ); ?></label><br/>
			<select id="<?php echo $this->get_field_id( 'layout' ); ?>" name="<?php echo $this->get_field_name( 'layout' ); ?>">
				<option <?php selected( $settings['layout'], 'horizontal' ); ?> value="horizontal" ><?php esc_html_e( 'Horizontal Arrangement', 'palm-beach-pro' ); ?></option>
				<option <?php selected( $settings['layout'], 'vertical' ); ?> value="vertical" ><?php esc_html_e( 'Vertical Arrangement', 'palm-beach-pro' ); ?></option>
			</select>
		</p>
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_palm_beach_magazine_posts_boxed', 'widget' );
