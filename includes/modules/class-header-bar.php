 ...
 * Header Bar Class
 */
class Palm_Beach_Pro_Header_Bar {
	...
	static function nav_menu_social_icons( $item_output, $item, $depth, $args ) {
		...
		// Get SVG.
		$svg = apply_filters( 'palm_beach_pro_get_social_svg', self::get_social_svg( $icon ), $item_output );
	...
	static function get_social_svg( $icon = null ) {
		...
		$svg .= ' <use xlink:href="' . PALM_BEACH_PRO_PLUGIN_URL . 'assets/icons/social-icons.svg?ver=20251219#icon-' . esc_html( $icon ) . '"></use> ';
		$svg .= '</svg>';
	...
	static function supported_social_icons() {
		...
		return apply_filters( 'palm_beach_pro_supported_social_icons', $supported_social_icons );
	}
...
// Run Class.
add_action( 'init', array( 'Palm_Beach_Pro_Header_Bar', 'setup' ) );

// Register navigation menus in backend.
add_action( 'after_setup_theme', array( 'Palm_Beach_Pro_Header_Bar', 'register_nav_menus' ), 20 );
